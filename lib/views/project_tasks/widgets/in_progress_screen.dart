...
class InProgressScreen extends StatelessWidget {
  ...
  const InProgressScreen({super.key, required this.projectId});
  ...
  Widget build(BuildContext context) {
    return ViewModelBuilder<ProjectTasksViewModel>.reactive(
        ...
        builder: (context, viewModel, child) {
          return Scaffold(
            body: FutureBuilder(
                future: viewModel.getInProgressTasks(projectId),
                builder: (context, AsyncSnapshot snapshot) {
                  if (snapshot.hasData) {
                    if (snapshot.data.docs.length == 0) {
                      return Center(
                        child: Text(
                          "${AppLocalizations.of(context)!.noTasksIn} ${AppLocalizations.of(context)!.inProgress}",
                          style: TextStyle(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return taskContainer(
                            ...
                            onMoveTask: () async {
                              ...
                              if (timerService.timers.containsKey(task.id)) {
